...
from ffis_chat import render_chat_tab
...
st.code(jn, language=None)
    ...
    # Quick create Salesforce org button
    st.divider()
    st.markdown("**Salesforce Org Management**")
    if st.button("🚀 Quick Create Dev Org", use_container_width=True, type="primary"):
        st.session_state["_show_quick_org_modal"] = True

    # Quick Create Org Modal/Dialog
    if st.session_state.get("_show_quick_org_modal"):
        st.markdown('<div class="section-header">⚡ Quick Create Salesforce Org</div>', unsafe_allow_html=True)
        
        modal_org_type = st.radio(
            "Choose org type:",
            ["🏗️ Development Org (Free, Permanent)", "🧪 Scratch Org (Temporary)"],
            key="modal_org_type"
        )
        
        # ─ Development Org Option ─────────────────────────────────────────────
        if "Development" in modal_org_type:
            st.caption("Free, permanent Salesforce org. No Dev Hub needed.")
            modal_dev_alias = st.text_input(
                "Org Alias",
                value="",
                placeholder="e.g. dev-org-1",
                key="modal_dev_alias",
                help="Name for your new Development org"
            )
            
            dev_col1, dev_col2 = st.columns(2)
            if dev_col1.button("📝 Sign Up Free", key="modal_dev_signup", help="Opens developer.salesforce.com/signup"):
                import webbrowser
                webbrowser.open("https://developer.salesforce.com/signup")
                st.info("✓ Signup page opened in browser. Complete the form, then come back to authorize.")
            
            if dev_col2.button("✅ Authorize", key="modal_dev_auth", type="primary"):
                if not modal_dev_alias:
                    st.error("Enter an org alias first.")
                else:
                    try:
                        from ffis_sf_org import login_web, _ss_record_call, list_orgs, _ss_replace_orgs
                        with st.spinner(f"Opening browser for login to **{modal_dev_alias}**…"):
                            res = login_web("https://login.salesforce.com", modal_dev_alias, set_default=True)
                        _ss_record_call(res)
                        if res.ok:
                            st.success(f"✅ **{modal_dev_alias}** authorized!")
                            st.balloons()
                            # Refresh orgs list
                            orgs, lr = list_orgs()
                            _ss_record_call(lr)
                            _ss_replace_orgs(orgs)
                            st.session_state["_show_quick_org_modal"] = False
                        else:
                            st.error(f"Auth failed: {res.parsed.get('message') if res.parsed else 'unknown error'}")
                    except Exception as e:
                        st.error(f"Error: {e}")
        
        # ─ Scratch Org Option ─────────────────────────────────────────────────
        else:
            st.caption("Temporary org (up to 30 days). Requires a Dev Hub.")
            quick_modal_alias = st.text_input(
                "Org Alias",
                value="",
                placeholder="e.g. scratch-1",
                key="modal_quick_alias",
                help="Name for your new scratch org"
            )
            
            # Get available dev hubs
            try:
                from ffis_sf_org import list_orgs, _ss_record_call, _ss_replace_orgs, _envelope_payload, create_scratch_org, DEFAULT_SCRATCH_DEF, SCRATCH_DURATION_DEFAULT, find_sf_cli
                if st.button("🔄 Check Dev Hubs", key="modal_check_devhubs"):
                    orgs, res = list_orgs()
                    _ss_record_call(res)
                    _ss_replace_orgs(orgs)
                
                orgs = st.session_state.get("sf_authorized_orgs") or []
                dev_hub_options = [o for o in orgs if o.is_dev_hub]
                
                if dev_hub_options:
                    st.success(f"✅ {len(dev_hub_options)} Dev Hub(s) found")
                else:
                    st.warning("⚠️ No Dev Hub — authorize one first in SF Org tab")
                    st.caption("Use: SF Org tab → Authorize Org (Web Login)")
                
                if st.button("🚀 Create Org", key="modal_create_btn", type="primary", use_container_width=True):
                    errs = []
                    if not quick_modal_alias:
                        errs.append("Enter an org alias.")
                    elif not dev_hub_options:
                        errs.append("No Dev Hub available.")
                    else:
                        import os
                        from pathlib import Path
                        quick_project_dir = os.getcwd()
                        if not Path(quick_project_dir).is_dir():
                            errs.append(f"Project directory not found: {quick_project_dir}")
                        elif not (Path(quick_project_dir) / "sfdx-project.json").is_file():
                            errs.append(f"Directory missing sfdx-project.json")
                    ...
                    if errs:
                        ...
                        with st.spinner(f"🚀 Creating **{quick_modal_alias}** (1–5 min)…"):
                            res = create_scratch_org(
                                project_dir=os.getcwd(),
                                definition_file=DEFAULT_SCRATCH_DEF,
                                alias=quick_modal_alias,
                                dev_hub_alias=dev_hub_options[0].alias or dev_hub_options[0].username,
                        ...
                        if res.ok:
                            ...
                            st.success(f"✅ Created **{quick_modal_alias}** (ID: {org_id})")
                            st.balloons()
                            st.session_state["_show_quick_org_modal"] = False
                            ...
                            msg = res.parsed.get("message") if res.parsed else ""
                            st.error(f"Failed: {msg or 'unknown error'}")
            except Exception as e:
                st.error(f"Error: {e}")
                st.caption("Make sure the SF Org tab is accessible")
